...
describe("formatSwissAddress", () => {
  ...
  it("formats a city with postcode only", () => {
    expect(formatSwissAddress({ city: "Genève", postcode: "1204" })).toBe(
      "1204 Genève",
    );
  });

  it("falls back through town/village/municipality/hamlet/county", () => {
    expect(formatSwissAddress({ town: "Altstätten", postcode: "9450" })).toBe(
      "9450 Altstätten",
    );
  ...
  it("uses pedestrian and neighbourhood/quarter fallbacks", () => {
    ...
    expect(formatSwissAddress({ neighbourhood: "Riethüsli" })).toBe(
      "Riethüsli",
    );
