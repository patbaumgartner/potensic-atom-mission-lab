...
describe("map.db generation", () => {
  ...
  it("chunks large missions into multiple flight records", async () => {
    const SQL = await loadSqlNode();
    const wps = Array.from({ length: 92 }, (_, i) => destinationPoint(ORIGIN, i * 3, 15 + i));
  ...
  it("preserves lat/lng order (multipointbean stores lat then lng)", async () => {
    ...
    const bytes = generateMapDb(SQL, [mission({ name: "order", waypoints: [wp, ORIGIN] })]);
    const parsed = parseMapDb(SQL, bytes);
  ...
  it("chunks each mission independently within a multi-mission db", async () => {
    const SQL = await loadSqlNode();
    const big = Array.from({ length: 50 }, (_, i) => destinationPoint(ORIGIN, i * 2, 15 + i));
    const bytes = generateMapDb(SQL, [
      mission({ name: "small" }),
    ...
    expect(parsed.records).toHaveLength(3);
    expect(parsed.records.map((r) => r.label)).toEqual(["small", "big 001-045", "big 046-050"]);
  });
  ...
  it("has no flight history for a freshly generated map.db", async () => {
    const SQL = await loadSqlNode();
    const bytes = generateMapDb(SQL, [mission()]);
    const parsed = parseMapDb(SQL, bytes);
    expect(parsed.flightHistory).toEqual([]);
  });

  it("parses real flown-flight summaries from flightnotes, newest first", async () => {
    const SQL = await loadSqlNode();
    const db = new SQL.Database(generateMapDb(SQL, []));
    const insNote = db.prepare(
      "INSERT INTO flightnotes (null_lpcolumn, distance, duration, height, speed, starttime) VALUES (?, ?, ?, ?, ?, ?)",
    );
    insNote.run([-1, 128.8, 304234, 16, 6.1, 1783342378090]);
    insNote.run([-1, 221.1, 44000, 20, 5, 1783342684442]);
    insNote.free();
    const bytes = db.export();
    db.close();

    const parsed = parseMapDb(SQL, bytes);
    expect(parsed.flightHistory).toHaveLength(2);
    // Newest (larger starttime) first.
    expect(parsed.flightHistory[0]).toEqual({
      id: 2,
      distanceM: 221.1,
      durationMs: 44000,
      heightM: 20,
      speedMs: 5,
      startedAtMs: 1783342684442,
    });
    expect(parsed.flightHistory[1].startedAtMs).toBe(1783342378090);
  });

  it("parses a flight history row with null fields as zeroes", async () => {
    const SQL = await loadSqlNode();
    const db = new SQL.Database(generateMapDb(SQL, []));
    db.run(
      "INSERT INTO flightnotes (null_lpcolumn, distance, duration, height, speed, starttime) VALUES (NULL, NULL, NULL, NULL, NULL, NULL)",
    );
    const bytes = db.export();
    db.close();
    const parsed = parseMapDb(SQL, bytes);
    expect(parsed.flightHistory).toHaveLength(1);
    expect(parsed.flightHistory[0]).toEqual({
      id: 1,
      distanceM: 0,
      durationMs: 0,
