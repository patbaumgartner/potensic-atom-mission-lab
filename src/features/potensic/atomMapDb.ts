...
export interface ParsedMapDb {
  ...
  /** Real flown-flight summaries logged by the device (most recent first). */
...
function applySchema(db: Database): void {
  ...
  const insSchema = db.prepare("INSERT INTO table_schema (name, type) VALUES (?, ?)");
  ...
  try {
    ...
    for (const mission of missions) {
      ...
      for (const chunk of chunks) {
        ...
        const durationSeconds = mission.plannedSpeedMs > 0 ? mileageM / mission.plannedSpeedMs : 0;
        insRecord.run([
          chunk.label,
...
export function parseMapDb(SQL: SqlJsStatic, bytes: Uint8Array): ParsedMapDb {
  ...
  try {
    ...
    return { userVersion, tables, records, flightHistory: parseFlightHistory(db) };
  } finally {
    db.close();
  }
}

/** Read `flightnotes` (real flown-flight summaries), most recent first. */
function parseFlightHistory(db: Database): FlightHistoryEntry[] {
  const notesRes = db.exec(
    "SELECT id, distance, duration, height, speed, starttime FROM flightnotes ORDER BY starttime DESC",
  );
  return (notesRes[0]?.values ?? []).map((row) => ({
    id: Number(row[0]),
    distanceM: Number(row[1] ?? 0),
    durationMs: Number(row[2] ?? 0),
    heightM: Number(row[3] ?? 0),
    speedMs: Number(row[4] ?? 0),
    startedAtMs: Number(row[5] ?? 0),
  }));
}
