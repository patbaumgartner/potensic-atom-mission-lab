...
/** Minimal shape shared by GeoJSON geometries, features, and feature collections. */
...
export function parseTrack(text: string, filename: string): ImportedTrack {
  ...
  } else if (lower.endsWith(".geojson") || lower.endsWith(".json") || trimmed.startsWith("{")) {
...
function parseGeoJSON(text: string): Waypoint[] {
  ...
  const handleGeom = (g: GeoJSONNode | undefined) => {
    ...
    switch (g.type) {
      ...
      case "Polygon":
        (g.coordinates as unknown[] | undefined)?.forEach(addCoords);
        break;
      case "Point":
        addCoords([g.coordinates]);
        break;
  ...
  const d = data as GeoJSONNode;
  if (d?.type === "FeatureCollection") {
    d.features?.forEach((f) => handleGeom((f as GeoJSONNode | undefined)?.geometry));
...
function parseCsv(text: string): Waypoint[] {
  ...
  let latIdx = header.findIndex((h) => h.startsWith("lat"));
